...
#        print(self.image_names)
        image_names_copy=self.image_names.copy()
...
#                    'invalid annotations file: {}: {}'.format(filename, e)), None)
        #print(len(self.image_names))
        self.image_names = image_names_copy
#        print(self.image_names)
        return len(self.image_names)
    ...
    def __parse_annotations(self, xml_root):
        ...
        # print(annotations)
        # print(np.array(annotations['bboxes']))
    ...
    def load_annotations(self, image_index):
        ...
        try:
            tree = ET.parse(os.path.join(
                self.data_dir, 'Annotations', filename))
            a=self.__parse_annotations(tree.getroot())
            if a['labels'].size ==0 :
                print(filename)
            return a
        except ET.ParseError as e:
